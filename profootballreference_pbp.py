...
def pullPlaybyPlay(season):
    ...
    for d in dates:
        ...
        day = d.split(" ")[1]
        day = day.zfill(2)
        mapping = {"September": "09", "October": "10", "November": "11", "December": "12", "January": "1",
                   "February": "2"}
        m = mapping[month]
        m = m.zfill(2)
        if (m == "01") | (m == "02"):
    ...
    dat = dat[
        ["Date", "awayteam", "hometeam", "awaypoints", "homepoints", "Detail", "Down", "ToGo", "Location", "Quarter",
         "Time", "nextpoints", "EPB", "EPA"]]
    dat["Season"] = season
...
def preparePlaybyPlay(dat):
    ...
    dat["pass"] = ((dat["Detail"].str.contains("pass")) | (dat["Detail"].str.contains("sacked"))) & (
    dat["challenge"] == False)
    ...
    dat["run"] = (dat["penalty"] == False) & (dat["challenge"] == False) & (dat["spike"] == False) & (
    dat["pass"] == False) & (dat["punt"] == False) & (dat["kickoff"] == False) & (dat["FG"] == False) & (
                 dat["xp"] == False) & (dat["to"] == False) & (dat["nodetail"] == False) & (dat["timeout"] == False)
    ...
    for t in teamnames2:
        ...
        carry = pullTable(url, "rushing_and_receiving", False)
        ...
        kick = pullTable(url, "kicking", False)
